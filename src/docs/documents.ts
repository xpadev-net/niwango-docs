...
const documents: IDocument[] = [
  {
    ...
    description: "値をログとして出力する関数です。複数の引数を指定することができます",
  },
  {
    name: "while_kari",
  ...
  {
    name: "distance",
    syntax: "distance(x1: number, y1: number, x2: number, y2: number) => number",
  ...
  },
  {
    ...
    description: "niwango-core.jsが読み込まれたタイミングのUnixミリ秒を返します"
  },
  {
    name: "timethis",
  ...
  },
  {
    ...
    description: "関数を定義します。Objectのメゾットとして定義することもできます",
    examples: ["def(test(obj),dump(obj));test(\"hello world\")","obj = Object.clone;obj.def(hoge(),dump(self.huga));obj.huga=5;obj.hoge()"]
  },
  {
    name: "def_kari",
  ...
  },
  {
    ...
    description: "コメントが再生されると呼び出されるイベントハンドラです。timerで有効期間を設定でき、省略した場合は無期限になります",
  },
  {
    name: "ctrig",
  ...
  {
    name: "drawShape",
    syntax: "drawShape(x: number, y: number, z: number, shape: 'rect'|'circle', width: number, height: number, color: number, visible: boolean, pos: string, mask: boolean, commentmask: boolean, alpha: number, rotation: number, mover: ''|'simple'|'smooth'|'rolling'|'kage')",
    description: "図形を描画する"
  },
  {
    name: "drawText",
    syntax: "drawText(text: string, x: number, y: number, z: number, size: number, pos: string, color: number, bold: boolean, visible: boolean, filter: ''|'fuchi'|'kasumi'|'kage', alpha: number, mover: ''|'simple'|'smooth'|'rolling'|'kage')",
